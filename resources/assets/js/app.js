...
imageInput.on('filebatchuploadsuccess', function(event, data, previewId, index) {
    ...
    if (response['templates'] && response['templates']['imageDetails']) {
        ...
        uploadForm.addClass('fade');

        // Wait for the transition effect, then insert our replaced HTML and fade back in
        setTimeout(function() {
            uploadForm.attr('id', 'image-details');
            uploadForm.html($imageDetails);
            uploadForm.addClass('in');
        }, 200);
    }
...
imageInput.on('filebatchuploaderror', function(event, data) {
    swal({
        title: 'Upload Failed',
...
// Center the users viewport on the specified element
function centerOn(element, speed) {
